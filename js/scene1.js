function chart_1(){
  ...
  var margin = {top: 100, right: 100, bottom: 90, left: 200},
  ...
  const annotations = [
    {
      note: {
        label: "Tax revenue from corporations is surprisingly small relative to the tax revenue from individuals.",
    ...
    {
      note: {
        label: "Click category to explore",
        title: ""
      },
      connector: {
  ...
  // Parse the Data
  d3.csv("../data/federal_revenue_categories.csv", function(data) {

    var data = data.filter((d)=>{return d.child !== '' && d.federal_revenue > 0;})
    var data = d3.nest()
      .key(function(d) { return d.parent;})
      .rollup(function(d) { 
      return d3.sum(d, function(g) {return g.federal_revenue; });
      }).entries(data);
...
function chart_2(key){
  // set the dimensions and margins of the graph
  var margin = {top: 100, right: 30, bottom: 90, left: 250},
      width = 600 - margin.left - margin.right,
      height = 450 - margin.top - margin.bottom;
